...
export class TelegramBotService {
  ...
  private lastTransactionTimes: { [chatId: number]: number } = {}; 
    ...
    this.bot.onText(/\/rpc/, (msg) => {
     ...
     this.bot.onText(/\/faucet(.+)?/, async (msg, match) => {
        ...
        // Extract the text after the '/faucet' command
         ...
         console.log(this.isWithinLimit(chatId))
      if (this.isWithinLimit(chatId)) {
        this.bot.sendMessage(chatId, 'You have reached the transaction limit. Please try again after 24 hours.');
        return;
      }
        ...
        try {
            ...
            if(transact) {
                this.bot.sendMessage(chatId, `Transaction successful for: ${trimmedText}`);
                this.lastTransactionTimes[chatId] = Date.now();
            } else {
                this.bot.sendMessage(chatId, `Transaction failled for: ${trimmedText}`);
            }
        } catch (error) {
            // If an error occurs, send error message
            this.bot.sendMessage(chatId, `Transaction failed for: ${trimmedText}. Error: ${error.message}`);
        }
      });
...
private isWithinLimit(chatId: number): boolean {
  console.log('Checking chatId:', chatId);
  const lastTransactionTime = this.lastTransactionTimes[chatId];
